...
It has always annoyed me, that Azure Functions does not have official support for Powershell
...
Azure Functions v2 has fixed the 6issues of sharing modules between functions. In combination with VS Code and the Azure Tool plugin, you can deploy your code with 99.95 % uptime in a few minutes. This blogpost will go through all the steps required.
...
Give your trigger a meaningfull name - HTTPTrigger1, etc is kind of hard to differentiate):
...
The trigger can be callled with get or post methods.
...
This takes you through a 5-8 step guide, that takes care of a lot of clicks in the Azure Protal. If you have an existing resource group and storage account, there are only 5 steps, if you need to create these resources, there will be 8.
...
Now that your function is deployed, get the function Url and test it. Start by right-clicking your function and choode Copy Function Url:
...
And yu will se, that your function returns the example code:

![Azure Tools Azure Function Done]({{ site.url }}/images/af1test.jpg)

## Adding modules to your function

In the previous version of Azure Functions for Powershell you either had to add a module folder for each function in your Azure function or youu could hack the PSModulePath variable.
